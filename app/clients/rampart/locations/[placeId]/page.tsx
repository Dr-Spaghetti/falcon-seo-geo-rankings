...
export function generateStaticParams() {
  return listClientPlaceIds("rampart").map((placeId) => ({ placeId }));
}
...
export function generateMetadata({
  ...
  return {
    title: city + " · Rampart Injury Lawyers · Falcon",
    description:
      "Local Falcon scans for " + city + " (Rampart Injury Lawyers)",
  };
}

export default function RampartLocationPage({
  params,
}: {
  params: { placeId: string };
}) {
  ...
  return (
    <AppShell wide>
      <div className="mb-6">
        ...
        >
          ← All Rampart Injury Lawyers locations
        </Link>
      </div>
