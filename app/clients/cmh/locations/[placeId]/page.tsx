...
export function generateStaticParams() {
  return listClientPlaceIds("cmh").map((placeId) => ({ placeId }));
}
...
export function generateMetadata({
  ...
  return {
    title: city + " · Carlson Hayslett · Falcon",
    description: "Local Falcon scans for " + city + " (Carlson Hayslett / CMH)",
  };
}

export default function CmhLocationPage({
  params,
}: {
  params: { placeId: string };
}) {
  ...
  return (
    <AppShell wide>
      <div className="mb-6">
        ...
        >
          ← All Carlson Hayslett locations
        </Link>
      </div>
