...
export function generateStaticParams() {
  return listClientPlaceIds("andy-callif").map((placeId) => ({ placeId }));
}
...
export function generateMetadata({
  ...
  return {
    title: city + " · Andy Callif Bail Bonds · Falcon",
    description: "Local Falcon scans for " + city + " (Andy Callif Bail Bonds)",
  };
}

export default function AndyCallifLocationPage({
  params,
}: {
  params: { placeId: string };
}) {
  ...
  return (
    <AppShell wide>
      <div className="mb-6">
        ...
        >
          ← All Andy Callif Bail Bonds locations
        </Link>
      </div>
