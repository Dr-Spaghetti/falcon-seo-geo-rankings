...
export function generateStaticParams() {
  return listClientPlaceIds("milano").map((placeId) => ({ placeId }));
}
...
export function generateMetadata({
  ...
  return {
    title: city + " · Milano Legal Group · Falcon",
    description:
      "Local Falcon scans for " + city + " (Milano Legal Group)",
  };
}

export default function MilanoLocationPage({
  params,
}: {
  params: { placeId: string };
}) {
  ...
  return (
    <AppShell wide>
      <div className="mb-6">
        ...
        >
          ← All Milano Legal Group locations
        </Link>
      </div>
