...
export function generateStaticParams() {
  return listClientPlaceIds("gold-dog").map((placeId) => ({ placeId }));
}
...
export function generateMetadata({
  ...
  return {
    title: city + " · Gold Dog Injury Law · Falcon",
    description: "Local Falcon scans for " + city + " (Gold Dog Injury Law)",
  };
}

export default function GoldDogLocationPage({
  params,
}: {
  params: { placeId: string };
}) {
  ...
  return (
    <AppShell wide>
      <div className="mb-6">
        ...
        >
          ← All Gold Dog Injury Law locations
        </Link>
      </div>
